...
export default function PaymentCheckout() {
  ...
  // Auto-poll while PENDING and not expired
  useEffect(() => {
  ...
  async function recreatePayment() {
    ...
    try {
      const payload = { amount: data.amount, currency: data.currency, buyerId: data.buyerId, companyId: data.companyId, memo: data.memo }
  ...
  async function manualRefresh() {
    ...
    try {
      ...
      setData(res)
    } catch (e) { setMsg(`${t('refresh_failed')}: ${e?.message || ''}`.trim()) } finally { setRefreshing(false) }
  }
  ...
  return (
    <div>
      ...
      <Modal open={confirmOpen} onClose={closeConfirm} title={t('confirm_create_title')}
        footer={(
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button onClick={closeConfirm}>{t('cancel')}</button>
            <button onClick={async () => { closeConfirm(); await recreatePayment() }} aria-busy={creating}>{t('confirm')}</button>
          </div>
        )}
      >
        <div>
