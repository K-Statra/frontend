        ...
        <div key={r.company._id} className="card" style={{ marginBottom: 12 }}>
          ...
          >
            ...
            <div>
              ...
              <div>
                {(r.reasons || []).find((x) => x.includes("recently")) || "-"}
              </div>
            </div>
          </div>
          <div style={{ marginTop: 8 }}>
            ...
            <textarea
              rows={3}
              placeholder="Leave your thoughts about this match (not saved yet)."
              style={{ width: "100%" }}
            />
          </div>
          <div className="row gap-4 mt-3">
            <Button
              variant="secondary"
              onClick={() => nav(`/companies/${r.company._id}`)}
            >
              Company Detail
            </Button>
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  const targetCompanyId =
                    companyId && /^[a-f0-9]{24}$/i.test(companyId)
                      ? companyId
                      : r.company._id;
                  setErrorMsg("");
