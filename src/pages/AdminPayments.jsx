  ...
  return (
    <div>
      ...
      {stats && (
        <div
          className="row"
          style={{ gap: 12, marginBottom: 8, flexWrap: "wrap" }}
        >
            ...
            <table className="table" style={{ marginTop: 8 }}>
              <thead>
                <tr>
                  ...
                  {["CREATED", "PENDING", "PAID", "FAILED", "CANCELLED"].map(
                    (s) => (
                      <th key={s}>{s}</th>
                    ),
                  )}
              ...
              <tbody>
                {Object.entries(stats.byCurrencyStatus || {}).map(
                  ([cur, map]) => {
                    const rowTotal = [
                      "CREATED",
                      "PENDING",
      ...
      <div className="row">
        ...
        />
        <button
          title={t("copy_current_link")}
          onClick={async () => {
            setError("");
            if (!isValidIso(filters.from) || !isValidIso(filters.to)) {
              setError(t("invalid_iso"));
              return;
            }
            if (!isValidRange(filters.from, filters.to)) {
              setError(t("range_invalid"));
              return;
            }
            updateQuery({ preset: presetSel });
            await load(1);
      ...
      <div className="row" style={{ gap: 8, marginTop: 8, flexWrap: "wrap" }}>
        ...
        {presetSel === "custom" && (
          <span className="muted">{t("custom_range")}</span>
        )}
      </div>
      ...
      >
        <label className="row" style={{ alignItems: "center", gap: 6 }}>
          <input
            type="checkbox"
            checked={clearUrlOnReset}
            onChange={(e) => setClearUrlOnReset(e.target.checked)}
          />
          <span className="muted">{t("clear_url_on_reset")}</span>
        </label>
      </div>
      <div className="muted" style={{ marginTop: 4 }}>
        {t("link_excludes_token")}
      </div>
      <div className="row" style={{ gap: 8, marginTop: 8 }}>
        <select
        ...
        <label className="row" style={{ alignItems: "center", gap: 6 }}>
          <input
            ...
            checked={tzMode === "utc"}
            onChange={(e) => {
              const mode = e.target.checked ? "utc" : "local";
              setTzMode(mode);
              try {
                localStorage.setItem("admin_tz_mode", mode);
              } catch (_) {}
              try {
                const params = new URLSearchParams({
                  ...Object.fromEntries(
        ...
        <input
          ...
          value={
            tzMode === "utc"
              ? isoToUtcInput(filters.from)
              : isoToLocalInput(filters.from)
          }
          onChange={(e) => {
            setFilters({
              ...filters,
        ...
        <input
          ...
          value={
            tzMode === "utc"
              ? isoToUtcInput(filters.to)
              : isoToLocalInput(filters.to)
          }
          onChange={(e) => {
            setFilters({
              ...filters,
      ...
      <div className="muted" style={{ marginTop: 4 }}>
        {t("tz_hint")} · <strong>{t("tz_label_local")}</strong> →{" "}
        <strong>{t("tz_label_server")}</strong>
      </div>
