...
export default function CompanyCard({ company, onDetails, onRequestPayment }) {
  ...
  const heroImage = hasImg
    ? c.images[0]
    : { url: PLACEHOLDER_IMAGE, alt: "K-Statra default image" };
  return (
    <Card>
      <img
        src={heroImage.url}
      ...
      {(onDetails || onRequestPayment || c.videoUrl) && (
        <div className="row gap-4 mt-3" style={{ flexWrap: "wrap" }}>
          ...
          {onRequestPayment && (
            <Button onClick={onRequestPayment}>결제 요청</Button>
          )}
          {c.videoUrl && (
            <Button
              variant="ghost"
              onClick={() =>
                window.open(c.videoUrl, "_blank", "noopener,noreferrer")
              }
            >
              소개 영상
            </Button>
